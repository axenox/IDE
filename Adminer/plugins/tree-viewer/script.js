...
var AdminerAjaxConnector = /** @class */ (function () {
    function AdminerAjaxConnector(searchParams) {
		var self = this;
		searchParams.forEach(function(v,p,i){
			switch (true) {
				case p === 'username':
					self.connectionUsername = v;
					break;
				case p === 'db':
					self.connectionDb = v;
					break;
				case p === 'ns':
					self.connectionNs = v;
					break;
				case self.connectionServer === undefined || self.connectionServer === null:
					self.connectionServer = p + '=' + v;
					break;
			}
		});
    ...
    AdminerAjaxConnector.prototype.urlFromSelectionQuery = function (selectionQuery) {
        ...
        if (this.connectionServer != null) {
            urlParts.push(this.connectionServer);
        }
        urlParts.push('username=' + this.connectionUsername);
        urlParts.push('db=' + this.connectionDb);
        if (this.connectionNs != null) {
            urlParts.push('ns=' + this.connectionNs);
        }
...
var HtmlGenerator = /** @class */ (function () {
    ...
    HtmlGenerator.prototype.createTableElementFromSelectionData = function (selectionQuery, selectionData) {
        var tableElement = HtmlGenerator._getTemplateAsElement('<table class="checkable">' +
            '   <thead>' +
            '       <tr>' +
