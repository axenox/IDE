...
add_driver("mssql_mod", "Microsoft SQL");
global $b;
global $adminer;
if (isset($_GET["mssql_mod"])) {
	define("DRIVER", "mssql_mod");
	...
	} elseif (extension_loaded("mssql_mod")) {
		class Min_DB {
			var $extension = "MSSQL_MOD", $_link, $_result, $server_info, $affected_rows, $error;
...
WHERE sys1.xtype = 'TR' AND sys2.name = " . q($table)
	...
	function driver_config() {
		...
		return array(
			'possible_drivers' => array("SQLSRV", "MSSQL", "MSSQL_MOD", "PDO_DBLIB"),
			'jush' => "mssql_mod",
			'types' => $types,
